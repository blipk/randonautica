...
var dialogs = require("tns-core-modules/ui/dialogs");
    ...
    export default {
        ...
        methods: {
            ...
            positionCompute: function(prop, positionPercent = [50, 0]) { //W, H
                ...
                //console.log("CHILD COMPONENTS: " + component.getChildrenCount());

                //console.dir(component, {'depth': null, 'maxArrayLength': null});
                //console.dir(component.getChildAt(0), {'depth': null, 'maxArrayLength': null});
                ...
                for(let i = 0; i < component.getChildrenCount(); i++) {
                    ...
                    let childEffectiveWidth = childWidth 
                        + parseFloat(child.marginLeft)
                        + parseFloat(child.marginRight)
                        + parseFloat(child.paddingLeft)
                        + parseFloat(child.paddingRight);

                    let childHeight = parseFloat(child.getActualSize().height);
                    let childEffectiveHeight = childHeight 
                    + parseFloat(child.marginBottom) 
                    + parseFloat(child.marginTop) 
                    + parseFloat(child.paddingBottom) 
                    + parseFloat(child.paddingTop);

                    
                    //console.log("- childprop: " + utils.layout.toDeviceIndependentPixels(child.effectiveWidth));
                    //console.log("- childprop: " + utils.layout.toDeviceIndependentPixels(child.getActualSize().width));
                    console.log("- childprop: " + parseInt(child.getActualSize().width));
                ...
                //console.log("- effectiveheight: " + component.getChildAt(1)._styleScope._mergedCssSelectors.list-group);
                console.log(maxWidth);

                positionPercent[0] = positionPercent[0]/100;
                positionPercent[1] = positionPercent[1]/100;

                let translateFromLeft = this.screenWidth*positionPercent[0] + maxWidth*positionPercent[0]*positionPercent[0];
                let translateFromTop = (this.screenHeight * positionPercent[1]) + maxHeight/2;

                let translateFromRight = this.screenWidth + maxWidth/2;
                let translateFromBottom = this.screenHeight - (this.screenHeight * (positionPercent[1]/100)) + maxHeight/2;
                ...
                //component.translateX = translateFromLeft; //.top also works
                //component.translateY = translateFromTop; //.left also works
            },
            test: function(prop) {
